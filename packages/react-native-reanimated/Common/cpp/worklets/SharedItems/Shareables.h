...
class MutableRawBuffer : public jsi::MutableBuffer {
 public:
  MutableRawBuffer(uint8_t *data, size_t size) : _data(data), _size(size) {}

  ~MutableRawBuffer() override = default;

  uint8_t *data() override {
    if (!_data)
      throw std::runtime_error("Attempting to access null data");
    return _data;
  }
